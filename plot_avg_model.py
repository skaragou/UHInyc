...
class AvgModelPlots:
    ...
    def coefficient_v_hour(self):
        titles = ['num_build500','min_distance_park','num_trees_15m','is_august','bias']
        ...
        for i,ax in enumerate(axes):
            ...
            for c, chain in enumerate(np.split(np.arange(4000),4)):
                ...
                ax.fill_between(range(24), y_min, y_max,alpha=0.1,color='red')
                ax.set_title(titles[i])
            df = pd.DataFrame(np.array(chain_mean).T,columns=[1,2,3,4])
            df['Hour'] = chain_x
            df = pd.melt(df,id_vars=['Hour'],value_vars=[1,2,3,4]).rename(columns={'variable':'Chain','value':'Value'})
            a = sns.lineplot(data=df,x='Hour',y='Value',hue='Chain',palette='crest',ax=ax,alpha=0.4)
            if i != 0:
                a.legend_.remove()
        fig.suptitle('Coefficient vs Hour, CI and Mean from 4 Chains',fontsize=20,y=0.94)
        plt.show()

    def samples_from_posterior_predictive(self):
        titles = ['Sample ' + str(i + 1) for i in range(5)]
        fig,axes = plt.subplots(5,1,figsize=(10,15),sharex=True)
        fig.suptitle('5 Samples and 95% CI from Posterior Predictive',fontsize=20,y=0.94)
        ...
        for i,ax in enumerate(axes):
            ...
            sns.lineplot(x=range(24),y=self.y_train[i,:],palette='crest',ax=ax)
    ...
    def checking_mcmc_convergence(self):
        ...
        fig.suptitle('Checking MCMC Convergence, 4 Chains',fontsize=20,y=0.90)
        for i in range(24):
            for j in range(5):
                for chain in np.split(np.arange(4000),4):
                    p = sns.lineplot(x=range(1000),y=self.b[chain,i,j],ax=axes[i,j])
    ...
    def hour_v_mse(self):
        mse = []
        for i in range(24):
            mse.append(mean_squared_error(self.y_val[:,i],np.mean(self.y_hat[:,:,i],axis=0)))
